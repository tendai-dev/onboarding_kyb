'use client';
/* eslint-disable @typescript-eslint/no-explicit-any */
import {
  Box,
  VStack,
  HStack,
  Flex,
  Icon,
  useDisclosure,
  Textarea,
} from '@chakra-ui/react';
import {
  Button,
  Typography,
  Tag,
  Modal,
  ModalHeader,
  ModalFooter,
} from '@/lib/mukuruImports';
import { FormControl, FormLabel } from '@/lib/mukuruComponentWrappers';
import { useState, useRef, useEffect } from 'react';
import {
  FiEdit3,
  FiCheck,
  FiX,
  FiDownload,
  FiEye,
  FiLock,
  FiUser,
  FiCalendar,
  FiClock,
  FiShield,
  FiRefreshCw,
  FiCheckCircle,
  FiInfo,
  FiAlertTriangle,
} from 'react-icons/fi';

export interface SignatureField {
  id: string;
  documentId: string;
  pageNumber: number;
  x: number;
  y: number;
  width: number;
  height: number;
  signerId: string;
  signerName: string;
  signerEmail: string;
  required: boolean;
  status: 'pending' | 'signed' | 'declined';
  signedAt?: string;
  signatureData?: string;
  signatureImage?: string;
  comments?: string;
}

export interface Document {
  id: string;
  name: string;
  type: 'agreement' | 'contract' | 'policy' | 'form';
  url: string;
  pages: number;
  signatureFields: SignatureField[];
  status:
    | 'draft'
    | 'pending_signatures'
    | 'partially_signed'
    | 'fully_signed'
    | 'declined';
  createdAt: string;
  updatedAt: string;
  expiresAt?: string;
  signers: Array<{
    id: string;
    name: string;
    email: string;
    role: 'primary' | 'secondary' | 'witness';
    status: 'pending' | 'signed' | 'declined';
  }>;
}

export interface SignatureSession {
  id: string;
  documentId: string;
  signerId: string;
  signerName: string;
  signerEmail: string;
  status: 'active' | 'completed' | 'expired';
  startedAt: string;
  completedAt?: string;
  ipAddress: string;
  userAgent: string;
  signatureData?: string;
  signatureImage?: string;
  auditTrail: Array<{
    action: string;
    timestamp: string;
    details: string;
  }>;
}

interface DigitalSignatureProps {
  document: Document;
  currentSigner: {
    id: string;
    name: string;
    email: string;
  };
  onSign: (signatureData: string, signatureImage: string) => Promise<void>;
  onDecline: (reason: string) => Promise<void>;
  onDownload: () => void;
  onViewDocument: () => void;
}

export function DigitalSignature({
  document,
  currentSigner,
  onSign,
  onDecline,
  onDownload,
  onViewDocument,
}: DigitalSignatureProps) {
  const [currentPage, setCurrentPage] = useState(1);
  const [isSigning, setIsSigning] = useState(false);
  const [signatureData, setSignatureData] = useState<string>('');
  const [signatureImage, setSignatureImage] = useState<string>('');
  const [declineReason, setDeclineReason] = useState('');
  const [showSignaturePad, setShowSignaturePad] = useState(false);
  const [selectedField, setSelectedField] = useState<SignatureField | null>(null);
  const [session, setSession] = useState<SignatureSession | null>(null);

  const canvasRef = useRef<HTMLCanvasElement>(null);
  const signaturePadRef = useRef<any>(null);

  const {
    open: isDeclineOpen,
    onOpen: onDeclineOpen,
    onClose: onDeclineClose,
  } = useDisclosure();
  // Preview disclosure not currently used
  // const {
  //   isOpen: isPreviewOpen,
  //   onOpen: onPreviewOpen,
  //   onClose: onPreviewClose,
  // } = useDisclosure();

  // Initialize signature session
  useEffect(() => {
    const newSession: SignatureSession = {
      id: `session_${Date.now()}`,
      documentId: document.id,
      signerId: currentSigner.id,
      signerName: currentSigner.name,
      signerEmail: currentSigner.email,
      status: 'active',
      startedAt: new Date().toISOString(),
      ipAddress: '192.168.1.1', // Mock IP
      userAgent: navigator.userAgent,
      auditTrail: [
        {
          action: 'session_started',
          timestamp: new Date().toISOString(),
          details: 'Signature session initiated',
        },
      ],
    };
    setSession(newSession);
  }, [document.id, currentSigner]);

  // Get signature fields for current page
  const currentPageFields = document.signatureFields.filter(
    (field) => field.pageNumber === currentPage
  );

  // Calculate signing progress
  const totalFields = document.signatureFields.length;
  const signedFields = document.signatureFields.filter(
    (field) => field.status === 'signed'
  ).length;
  const progressPercentage = totalFields > 0 ? (signedFields / totalFields) * 100 : 0;

  const handleSignatureStart = (_field: Record<string, unknown>) => {
    const field = _field as unknown as SignatureField;
    setSelectedField(field);
    setShowSignaturePad(true);

    // Add to audit trail
    if (session) {
      setSession((prev) => ({
        ...prev!,
        auditTrail: [
          ...prev!.auditTrail,
          {
            action: 'signature_started',
            timestamp: new Date().toISOString(),
            details: `Started signing field: ${field.id}`,
          },
        ],
      }));
    }
  };

  const handleSignatureComplete = async () => {
    if (!signatureData || !selectedField) return;

    setIsSigning(true);
    try {
      await onSign(signatureData, signatureImage);

      // Update session
      if (session) {
        setSession((prev) => ({
          ...prev!,
          auditTrail: [
            ...prev!.auditTrail,
            {
              action: 'signature_completed',
              timestamp: new Date().toISOString(),
              details: `Completed signing field: ${selectedField.id}`,
            },
          ],
        }));
      }

      setShowSignaturePad(false);
      setSelectedField(null);
      setSignatureData('');
      setSignatureImage('');
    } catch (error) {
      console.error('Error signing document:', error);
    } finally {
      setIsSigning(false);
    }
  };

  const handleDecline = async () => {
    if (!declineReason.trim()) return;

    try {
      await onDecline(declineReason);
      onDeclineClose();
    } catch (error) {
      console.error('Error declining document:', error);
    }
  };

  const clearSignature = () => {
    setSignatureData('');
    setSignatureImage('');
    if (signaturePadRef.current) {
      signaturePadRef.current.clear();
    }
  };

  const getStatusColor = (_status: Record<string, unknown>) => {
    const status = typeof _status === 'string' ? _status : String(_status);
    switch (status) {
      case 'signed':
        return 'green';
      case 'pending':
        return 'orange';
      case 'declined':
        return 'red';
      default:
        return 'gray';
    }
  };

  const getStatusIcon = (_status: Record<string, unknown>) => {
    const status = typeof _status === 'string' ? _status : String(_status);
    switch (status) {
      case 'signed':
        return FiCheckCircle;
      case 'pending':
        return FiClock;
      case 'declined':
        return FiX;
      default:
        return FiClock;
    }
  };

  return (
    <Box minH="100vh" bg="gray.50">
      {/* Header */}
      <Box bg="white" borderBottom="1px" borderColor="gray.200" p="6">
        <VStack gap="4" align="stretch">
          <HStack justify="space-between">
            <VStack align="start" gap="1">
              <Typography fontSize="2xl" fontWeight="bold" color="gray.800">
                Digital Signature
              </Typography>
              <Typography color="gray.600">
                {document.name} • {document.type.toUpperCase()}
              </Typography>
            </VStack>

            <HStack gap="3">
              <Button variant="ghost" onClick={onViewDocument}>
                <Icon as={FiEye} mr="2" />
                View Document
              </Button>
              <Button variant="ghost" onClick={onDownload}>
                <Icon as={FiDownload} mr="2" />
                Download
              </Button>
            </HStack>
          </HStack>

          {/* Progress */}
          <Box>
            <HStack justify="space-between" mb="2">
              <Typography fontSize="sm" fontWeight="medium" color="gray.700">
                Signing Progress
              </Typography>
              <Typography fontSize="sm" color="gray.600">
                {signedFields} of {totalFields} fields signed
              </Typography>
            </HStack>
            <Box
              width="100%"
              height="8px"
              bg="gray.200"
              borderRadius="full"
              overflow="hidden"
            >
              <Box
                height="100%"
                width={`${progressPercentage}%`}
                bg="primary.500"
                transition="width 0.3s"
              />
            </Box>
            <Typography fontSize="xs" color="gray.500" mt="1" textAlign="center">
              {Math.round(progressPercentage)}% Complete
            </Typography>
          </Box>

          {/* Signer Info */}
          <HStack justify="space-between" p="4" bg="primary.50" borderRadius="lg">
            <HStack gap="3">
              <Icon as={FiUser} color="primary.600" boxSize="5" />
              <VStack align="start" gap="0">
                <Typography fontWeight="medium" color="primary.800">
                  {currentSigner.name}
                </Typography>
                <Typography fontSize="sm" color="primary.600">
                  {currentSigner.email}
                </Typography>
              </VStack>
            </HStack>

            <HStack gap="4">
              <VStack align="center" gap="0">
                <Icon as={FiCalendar} color="primary.600" boxSize="4" />
                <Typography fontSize="xs" color="primary.600">
                  {new Date().toLocaleDateString()}
                </Typography>
              </VStack>
              <VStack align="center" gap="0">
                <Icon as={FiShield} color="primary.600" boxSize="4" />
                <Typography fontSize="xs" color="primary.600">
                  Secure
                </Typography>
              </VStack>
            </HStack>
          </HStack>
        </VStack>
      </Box>

      {/* Main Content */}
      <Flex>
        {/* Document Viewer */}
        <Box flex="1" p="6">
          <VStack gap="4" align="stretch">
            {/* Page Navigation */}
            <HStack justify="center" gap="2">
              <Button
                size="sm"
                variant="ghost"
                onClick={() => setCurrentPage(Math.max(1, currentPage - 1))}
                disabled={currentPage === 1}
              >
                Previous
              </Button>
              <Typography fontSize="sm" color="gray.600">
                Page {currentPage} of {document.pages}
              </Typography>
              <Button
                size="sm"
                variant="ghost"
                onClick={() => setCurrentPage(Math.min(document.pages, currentPage + 1))}
                disabled={currentPage === document.pages}
              >
                Next
              </Button>
            </HStack>

            {/* Document Preview */}
            <Box
              bg="white"
              borderRadius="lg"
              boxShadow="sm"
              border="1px"
              borderColor="gray.200"
              p="4"
              position="relative"
              minH="600px"
            >
              {/* Mock document content */}
              <VStack gap="4" align="stretch">
                <Typography fontSize="lg" fontWeight="bold" textAlign="center">
                  {document.name}
                </Typography>
                <Box height="1px" bg="gray.200" />
                <Typography fontSize="sm" color="gray.600">
                  This is a preview of page {currentPage} of the document. In a real
                  implementation, this would show the actual document content.
                </Typography>

                {/* Signature Fields Overlay */}
                {currentPageFields.map((field) => (
                  <Box
                    key={field.id}
                    position="absolute"
                    left={`${field.x}%`}
                    top={`${field.y}%`}
                    width={`${field.width}%`}
                    height={`${field.height}%`}
                    border="2px"
                    borderColor={
                      field.status === 'signed' ? 'success.500' : 'primary.500'
                    }
                    borderStyle={field.status === 'signed' ? 'solid' : 'dashed'}
                    borderRadius="md"
                    bg={field.status === 'signed' ? 'success.50' : 'primary.50'}
                    cursor={field.status === 'pending' ? 'pointer' : 'default'}
                    onClick={() =>
                      field.status === 'pending' &&
                      handleSignatureStart(field as unknown as Record<string, unknown>)
                    }
                    _hover={field.status === 'pending' ? { bg: 'primary.100' } : {}}
                    transition="all 0.2s"
                  >
                    <VStack height="100%" justify="center" align="center" gap="1" p="2">
                      {field.status === 'signed' ? (
                        <>
                          <Icon as={FiCheckCircle} color="success.500" boxSize="4" />
                          <Typography
                            fontSize="xs"
                            color="success.600"
                            textAlign="center"
                          >
                            Signed by {field.signerName}
                          </Typography>
                          <Typography fontSize="xs" color="success.500">
                            {new Date(field.signedAt!).toLocaleDateString()}
                          </Typography>
                        </>
                      ) : (
                        <>
                          <Icon as={FiEdit3} color="primary.500" boxSize="4" />
                          <Typography
                            fontSize="xs"
                            color="primary.600"
                            textAlign="center"
                          >
                            Click to sign
                          </Typography>
                          <Typography fontSize="xs" color="primary.500">
                            {field.signerName}
                          </Typography>
                        </>
                      )}
                    </VStack>
                  </Box>
                ))}
              </VStack>
            </Box>
          </VStack>
        </Box>

        {/* Sidebar */}
        <Box width="350px" bg="white" borderLeft="1px" borderColor="gray.200" p="6">
          <VStack gap="6" align="stretch">
            {/* Signing Instructions */}
            <Box>
              <Typography fontSize="lg" fontWeight="semibold" color="gray.800" mb="3">
                Signing Instructions
              </Typography>
              <VStack gap="3" align="stretch">
                <HStack gap="3">
                  <Icon as={FiInfo} color="info.500" boxSize="4" />
                  <Typography fontSize="sm" color="gray.600">
                    Review the document carefully before signing
                  </Typography>
                </HStack>
                <HStack gap="3">
                  <Icon as={FiShield} color="success.500" boxSize="4" />
                  <Typography fontSize="sm" color="gray.600">
                    Your signature is legally binding
                  </Typography>
                </HStack>
                <HStack gap="3">
                  <Icon as={FiLock} color="primary.500" boxSize="4" />
                  <Typography fontSize="sm" color="gray.600">
                    All actions are recorded for audit purposes
                  </Typography>
                </HStack>
              </VStack>
            </Box>

            {/* Signature Fields Status */}
            <Box>
              <Typography fontSize="lg" fontWeight="semibold" color="gray.800" mb="3">
                Signature Fields
              </Typography>
              <VStack gap="2" align="stretch">
                {document.signatureFields.map((field) => (
                  <HStack
                    key={field.id}
                    p="3"
                    bg={field.pageNumber === currentPage ? 'primary.50' : 'gray.50'}
                    borderRadius="md"
                    border={field.pageNumber === currentPage ? '1px' : 'none'}
                    borderColor={
                      field.pageNumber === currentPage ? 'primary.200' : 'transparent'
                    }
                  >
                    <Icon
                      as={getStatusIcon(
                        field.status as unknown as Record<string, unknown>
                      )}
                      color={`${getStatusColor(field.status as unknown as Record<string, unknown>)}.500`}
                      boxSize="4"
                    />
                    <VStack align="start" gap="0" flex="1">
                      <Typography fontSize="sm" fontWeight="medium" color="gray.800">
                        Page {field.pageNumber}
                      </Typography>
                      <Typography fontSize="xs" color="gray.600">
                        {field.signerName}
                      </Typography>
                    </VStack>
                    <Tag variant="solid" size="md">
                      {field.status}
                    </Tag>
                  </HStack>
                ))}
              </VStack>
            </Box>

            {/* Action Buttons */}
            <VStack gap="3" align="stretch">
              <Button
                colorScheme="primary"
                size="md"
                onClick={() => {
                  const pendingField = document.signatureFields.find(
                    (f) => f.status === 'pending'
                  );
                  if (pendingField) {
                    handleSignatureStart(
                      pendingField as unknown as Record<string, unknown>
                    );
                  }
                }}
                disabled={document.signatureFields.every((f) => f.status !== 'pending')}
              >
                <Icon as={FiEdit3} mr="2" />
                Sign Document
              </Button>

              <Button variant="ghost" colorScheme="error" onClick={onDeclineOpen}>
                <Icon as={FiX} mr="2" />
                Decline to Sign
              </Button>
            </VStack>

            {/* Audit Trail */}
            {session && (
              <Box>
                <Typography fontSize="lg" fontWeight="semibold" color="gray.800" mb="3">
                  Audit Trail
                </Typography>
                <VStack gap="2" align="stretch" maxH="200px" overflowY="auto">
                  {session.auditTrail.map((entry, index) => (
                    <HStack key={index} gap="2" align="start">
                      <Icon as={FiClock} color="gray.400" boxSize="3" mt="1" />
                      <VStack align="start" gap="0" flex="1">
                        <Typography fontSize="xs" fontWeight="medium" color="gray.700">
                          {entry.action.replace('_', ' ').toUpperCase()}
                        </Typography>
                        <Typography fontSize="xs" color="gray.500">
                          {new Date(entry.timestamp).toLocaleString()}
                        </Typography>
                      </VStack>
                    </HStack>
                  ))}
                </VStack>
              </Box>
            )}
          </VStack>
        </Box>
      </Flex>

      {/* Signature Pad Modal */}
      <Modal
        isOpen={showSignaturePad}
        onClose={() => setShowSignaturePad(false)}
        size="large"
        header={<ModalHeader>Sign Document - {selectedField?.signerName}</ModalHeader>}
        footer={
          <ModalFooter>
            <HStack justify="space-between" width="full">
              <Button variant="ghost" onClick={clearSignature}>
                <Icon as={FiRefreshCw} mr="2" />
                Clear
              </Button>
              <HStack>
                <Button variant="ghost" onClick={() => setShowSignaturePad(false)}>
                  Cancel
                </Button>
                <Button
                  colorScheme="primary"
                  onClick={handleSignatureComplete}
                  loading={isSigning}
                  disabled={!signatureData}
                >
                  <Icon as={FiCheck} mr="2" />
                  Sign & Complete
                </Button>
              </HStack>
            </HStack>
          </ModalFooter>
        }
      >
        <VStack gap="4" align="stretch">
          <Box
            p="3"
            bg="info.50"
            borderLeft="4px"
            borderColor="info.500"
            borderRadius="md"
          >
            <HStack>
              <Icon as={FiInfo} color="info.500" boxSize="5" />
              <Typography fontSize="sm" color="info.800">
                Please sign in the box below. Your signature will be legally binding.
              </Typography>
            </HStack>
          </Box>

          {/* Signature Canvas */}
          <Box
            border="2px"
            borderColor="gray.300"
            borderRadius="md"
            bg="white"
            minH="200px"
            position="relative"
          >
            <canvas
              ref={canvasRef}
              width={600}
              height={200}
              style={{
                width: '100%',
                height: '200px',
                cursor: 'crosshair',
                borderRadius: '6px',
              }}
            />

            {!signatureData && (
              <Box
                position="absolute"
                top="50%"
                left="50%"
                transform="translate(-50%, -50%)"
                color="gray.400"
                pointerEvents="none"
              >
                <Typography fontSize="sm">Sign here</Typography>
              </Box>
            )}
          </Box>
        </VStack>
      </Modal>

      {/* Decline Modal */}
      <Modal
        isOpen={isDeclineOpen}
        onClose={onDeclineClose}
        header={<ModalHeader>Decline to Sign</ModalHeader>}
        footer={
          <ModalFooter>
            <HStack>
              <Button variant="ghost" onClick={onDeclineClose}>
                Cancel
              </Button>
              <Button
                colorScheme="error"
                onClick={handleDecline}
                disabled={!declineReason.trim()}
              >
                Decline to Sign
              </Button>
            </HStack>
          </ModalFooter>
        }
      >
        <VStack gap="4" align="stretch">
          <Box
            p="3"
            bg="warning.50"
            borderLeft="4px"
            borderColor="warning.500"
            borderRadius="md"
          >
            <VStack align="start" gap="2">
              <HStack>
                <Icon as={FiAlertTriangle} color="warning.500" boxSize="5" />
                <Typography fontWeight="semibold" fontSize="sm" color="warning.800">
                  Are you sure?
                </Typography>
              </HStack>
              <Typography fontSize="sm" color="warning.700">
                Declining to sign this document will cancel the signing process. This
                action cannot be undone.
              </Typography>
            </VStack>
          </Box>

          <FormControl>
            <FormLabel>Reason for declining (required)</FormLabel>
            <Textarea
              placeholder="Please provide a reason for declining to sign..."
              value={declineReason}
              onChange={(e) => setDeclineReason(e.target.value)}
              rows={4}
            />
          </FormControl>
        </VStack>
      </Modal>
    </Box>
  );
}
