...
import {
  ClientOnlyMukuruLogo,
  ClientOnlyMukuruButton,
  ClientOnlyMukuruTypography,
} from '../MukuruComponents';

// Mock mukuruImports
vi.mock('@/lib/mukuruImports', () => ({
  MukuruLogo: ({ ...props }: unknown) => (
    // eslint-disable-next-line react/jsx-props-no-spreading
    <div data-testid="mukuru-logo" {...props}>
      Logo
    </div>
  ),
  Button: ({ children, ...props }: unknown) => (
    // eslint-disable-next-line react/jsx-props-no-spreading
    <button data-testid="mukuru-button" {...props}>
      {children}
    </button>
  ),
  Typography: ({ children, ...props }: unknown) => (
    // eslint-disable-next-line react/jsx-props-no-spreading
    <p data-testid="mukuru-typography" {...props}>
      {children}
    </p>
  ),
}));
...
describe('MukuruComponents', () => {
  ...
  describe('ClientOnlyMukuruButton', () => {
    it('should render Button component with children', () => {
      const { getByTestId, getByText } = render(
        <ClientOnlyMukuruButton>Click me</ClientOnlyMukuruButton>
      );
    ...
    it('should pass props to Button', () => {
      const { getByTestId } = render(
        <ClientOnlyMukuruButton onClick={() => {}} disabled>
          Button
        </ClientOnlyMukuruButton>
      );
  ...
  describe('ClientOnlyMukuruTypography', () => {
    it('should render Typography component with children', () => {
      const { getByTestId, getByText } = render(
        <ClientOnlyMukuruTypography>Hello World</ClientOnlyMukuruTypography>
      );
    ...
    it('should pass props to Typography', () => {
      const { getByTestId } = render(
        <ClientOnlyMukuruTypography variant="h1">Title</ClientOnlyMukuruTypography>
      );
