...
describe('PerformanceOptimizations', () => {
  ...
  describe('VirtualScroll', () => {
    ...
    it('should handle scrolling', () => {
      ...
      const scrollElement =
        container.querySelector('[data-testid="virtual-scroll"]') || container;
      ...
      const { container } = renderWithProviders(
        <VirtualScroll
          ...
          keyExtractor={(item: unknown, index: number) => {
            const typedItem = item as { id?: string | number } | null | undefined;
            return String(typedItem?.id ?? index?.toString() ?? '');
          }}
        />
      );
      expect(container).toBeInTheDocument();
  ...
  describe('LazyImage', () => {
    it('should render LazyImage component', () => {
      const { container } = renderWithProviders(<LazyImage src="test.jpg" alt="Test" />);
      expect(container).toBeInTheDocument();
    });
