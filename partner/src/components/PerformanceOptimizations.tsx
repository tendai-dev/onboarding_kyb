...
import { Box, VStack, HStack, Spinner, Flex, Icon } from '@chakra-ui/react';
import { Typography, Checkbox, Input } from '@/lib/mukuruImports';
import { FiChevronDown, FiArrowUp, FiArrowDown, FiSearch, FiInfo } from 'react-icons/fi';
  ...
  return (
    ...
    >
      <Box height={totalHeight} position="relative">
        <Box position="absolute" top={offsetY} width="100%">
          {visibleItems.map((item, index) => (
            <Box key={keyExtractor(item, visibleRange.start + index)} height={itemHeight}>
  ...
  return (
    ...
    >
      {isVisible ? (
        ...
        placeholder || (
          <VStack gap="4">
            <Spinner size="lg" color="info.500" />
...
interface TableColumn<T> {
  ...
  width?: string;
  sortable?: boolean;
  render?: (value: Record<string, unknown>, item: T, index: number) => React.ReactNode;
}
...
export function MemoizedTable<T extends { id: string }>({
  ...
  const renderRow = useCallback(
    (item: T, index: number) => (
      <Flex
        ...
        bg={selectedRows.includes(item.id) ? 'info.50' : 'transparent'}
      >
        {selectable && (
          <Checkbox
            checked={selectedRows.includes(item.id)}
            onCheckedChange={(details: { checked: string | boolean }) =>
              onRowSelect?.(item, details.checked === true || details.checked === 'true')
            }
            mr="3"
        ...
        {columns.map((column) => (
          <Box key={String(column.key)} flex="1" minWidth="0" pr="4" width={column.width}>
            {column.render ? (
              column.render(item[column.key] as Record<string, unknown>, item, index)
            ) : (
              <Typography fontSize="sm" lineClamp={1}>
  ...
  if (loading) {
    return (
      <VStack gap="4" py="8">
        <Spinner size="lg" color="info.500" />
        <Typography color="mukuru.grey.medium">Loading data...</Typography>
      </VStack>
    );
  }
  ...
  if (data.length === 0) {
    return (
      <VStack gap="4" py="8">
        <Icon as={FiInfo} boxSize="8" color="mukuru.grey.medium" />
        <Typography color="mukuru.grey.medium">{emptyMessage}</Typography>
      </VStack>
    );
  }
      ...
      >
        ...
        {columns.map((column) => (
          <Box
            ...
            _hover={column.sortable ? { color: 'info.500' } : {}}
          >
            <HStack gap="1">
              <Typography color="mukuru.text.primary">{column.label}</Typography>
              {column.sortable && (
                <Icon
                  as={
                  ...
                  boxSize="3"
                  color={internalSortKey === column.key ? 'info.500' : 'gray.400'}
                />
              )}
            </HStack>
          </Box>
      ...
      <HStack>
        ...
        <Box position="absolute" right="2" top="50%" transform="translateY(-50%)">
          {loading ? (
            ...
            <Icon as={FiSearch} color="mukuru.grey.medium" />
          )}
        </Box>
      </HStack>
  ...
  return (
    ...
    >
      <VStack gap="1" align="start">
        <Typography color="mukuru.text.primary">FPS: {performanceData.fps}</Typography>
        <Typography color="mukuru.text.primary">
          Memory: {performanceData.memoryUsage.toFixed(1)}MB
        </Typography>
        <Typography color="mukuru.text.primary">
          Render: {performanceData.renderTime.toFixed(2)}ms
        </Typography>
        <Typography color="mukuru.text.primary">
          Components: {performanceData.componentCount}
        </Typography>
      </VStack>
    </Box>
  );
...
export function useBundleOptimization() {
  ...
  return {
    ...
    optimizeBundle: () => {
      ...
      console.info('Bundle optimization triggered');
    },
  };
}
