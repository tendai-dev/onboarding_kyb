...
export const documentValidation = {
  validateFileType: (file: File, acceptedTypes?: string[]) => {
    const typesToCheck = acceptedTypes || allowedTypes;
    return typesToCheck.includes(file.type);
  },

  validateFileSize: (file: File, maxSizeInMB?: number) => {
    const sizeToCheck = maxSizeInMB || maxSizeMB;
    const maxSizeBytes = sizeToCheck * 1024 * 1024;
  ...
  getRequiredDocuments: (entityType: string, isEPP: boolean = false) => {
    ...
    const result = documentRequirements[entityType as keyof typeof documentRequirements];
    return result || [];
  },
...
export const riskAssessmentValidation = {
  calculateRiskScore: (data: {
    ...
    const countryScore =
      countryRiskScores[data.country as keyof typeof countryRiskScores] || 50;
    ...
    const entityScore =
      entityRiskScores[data.entityType as keyof typeof entityRiskScores] || 50;
    ...
    const volumeScore =
      volumeRanges[data.expectedVolume as keyof typeof volumeRanges] || 50;
    score += volumeScore * 0.3;
    maxScore += 100 * 0.3;

    // Documentation completeness (weight: 25%)
    const isEPP = typeof data.isEPP === 'boolean' ? data.isEPP : false;
    const requiredDocs = documentValidation.getRequiredDocuments(data.entityType, isEPP);
    const uploadedDocs = (data.uploadedDocuments as unknown[]) || [];
    const docsArray = Array.isArray(requiredDocs) ? requiredDocs : [];
    const completeness =
      docsArray.length > 0 ? (uploadedDocs.length / docsArray.length) * 100 : 0;
