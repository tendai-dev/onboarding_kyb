...
describe('validation utilities', () => {
  ...
  describe('documentValidation', () => {
    ...
    it('should validate file types correctly', () => {
      ...
      expect(documentValidation.validateFileType(pdfFile, ['application/pdf'])).toBe(
        true
      );
      expect(documentValidation.validateFileType(jpgFile, ['application/pdf'])).toBe(
        false
      );
    });
    ...
    it('should validate file sizes correctly', () => {
      ...
      const largeFile = new File(new Array(6 * 1024 * 1024).fill('a'), 'large.pdf', {
  ...
  describe('validation schema tests', () => {
    it('should validate pty_ltd company name', async () => {
      ...
      await expect(
        schema.validate({ companyName: 'Test Company' })
