...
describe('Test Token Exchange API Route', () => {
  ...
  it('should return error if code is missing', async () => {
    const req = new NextRequest('http://localhost/api/auth/test-token-exchange', {
      method: 'GET',
    });
    await GET(req);
  ...
  it('should attempt token exchange with code', async () => {
    ...
    const req = new NextRequest(
      'http://localhost/api/auth/test-token-exchange?code=test-code',
      { method: 'GET' }
    );
    await GET(req);
    const data = await response.json();
  ...
  it('should handle token exchange errors', async () => {
    ...
    const req = new NextRequest(
      'http://localhost/api/auth/test-token-exchange?code=invalid-code',
      { method: 'GET' }
    );
    await GET(req);
    const data = await response.json();
