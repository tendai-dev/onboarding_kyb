...
describe('integrationService', () => {
  ...
  describe('getChecklistByCase', () => {
    it('should fetch checklist by case ID', async () => {
      const mockChecklist = {
        id: 'checklist-1',
        caseId: 'case-1',
        items: [{ id: 'item-1', name: 'Item 1', category: 'Compliance' }],
      };
    ...
    it('should complete checklist item', async () => {
      ...
      expect(global.fetch).toHaveBeenCalledWith(
        expect.stringContaining(
          '/api/proxy/api/v1/checklists/checklist-1/items/item-1/complete'
        ),
  ...
  describe('updateWorkQueueItem', () => {
    it('should update work queue item', async () => {
      ...
      expect(global.fetch).toHaveBeenCalledWith(
        expect.stringContaining(
          '/api/proxy/api/v1/workqueue/items/by-application/item-1'
        ),
        expect.objectContaining({
          method: 'PATCH',
        })
