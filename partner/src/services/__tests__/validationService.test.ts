...
describe('validationService', () => {
  ...
  describe('validateCompanyRegistration', () => {
    ...
    it('should use cache when enabled', async () => {
      ...
      const result1 = await validationService.validateCompanyRegistration(
        '123456',
        'ZA',
        config
      );
      const result2 = await validationService.validateCompanyRegistration(
        '123456',
        'ZA',
        config
      );
  ...
  describe('validateBankAccount', () => {
    it('should validate bank account', async () => {
      const result = await validationService.validateBankAccount(
        '1234567890',
        'BANK001',
        'ZA'
      );
    ...
    it('should use cache when enabled', async () => {
      ...
      const result1 = await validationService.validateBankAccount(
        '1234567890',
        'BANK001',
        'ZA',
        config
      );
      const result2 = await validationService.validateBankAccount(
        '1234567890',
        'BANK001',
        'ZA',
        config
  ...
  describe('validatePhoneNumber', () => {
    ...
    it('should use cache when enabled', async () => {
      ...
      const result1 = await validationService.validatePhoneNumber(
        '+27123456789',
        'ZA',
        config
      );
      const result2 = await validationService.validatePhoneNumber(
        '+27123456789',
        'ZA',
        config
      );
    ...
    it('should handle unknown validation type', async () => {
      const validations = [
        { type: 'email' as const, data: { email: 'test@example.com' } },
        {
          ...
          data: { registrationNumber: '123456', country: 'ZA' },
        },
      ];
      const results = await validationService.validateBatch(validations);
      ...
      expect(results).toHaveLength(2);
      expect(
        results.every((r) => Object.prototype.hasOwnProperty.call(r, 'isValid'))
      ).toBe(true);
    });
  });
