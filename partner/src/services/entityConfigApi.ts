...
class EntityConfigApiService {
  private async request<T>(endpoint: string, options?: RequestInit): Promise<T> {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `Entity Config API request failed: ${response.status} ${response.statusText} - ${errorText}`
        );
      }
      ...
      return transformKeys(json as Record<string, unknown>) as T;
    } catch (error) {
      if (
  ...
  async getWizardConfigurationByEntityType(
    ...
    try {
      return this.request<WizardConfiguration>(
