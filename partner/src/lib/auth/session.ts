...
export async function getAccessToken(): Promise<string | null> {
  ...
  console.warn(
    '[SECURITY] getAccessToken() called - tokens are server-side only. Use API proxy instead.'
  );
  return null;
}
...
export async function refreshToken(): Promise<boolean> {
  ...
  console.warn(
    '[SECURITY] refreshToken() called - token refresh is handled server-side by NextAuth'
  );
  return false;
}
 ...
 */
export function isTokenExpired(_bufferSeconds: number = 120): boolean {
