...
import { sendEmailNotification } from './emailTemplates';
import { sendCustomNotification } from './notificationService';
...
<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
  ...
  try {
    const success = await sendCustomNotification(to, subject, htmlContent, textContent);
    return { success };
  } catch (error) {
...
export async function testAllEmailTemplates(to: string): Promise<{
  ...
  acknowledgement: { success: boolean; error?: string };
  ...
  const results = {
    welcome: await sendEmailNotification(
      to,
      'Test: Welcome Email',
      'welcome',
      testData
    ...
    acknowledgement: await sendEmailNotification(
      to,
