...
// TTLs (in seconds)
const ACCOUNT_TTL = 7 * 24 * 60 * 60; // 7 days
...
export function RedisAdapter(): Adapter {
  ...
  return {
    ...
    async updateUser(
      ...
      if (process.env.NODE_ENV === 'development') {
        console.info('[RedisAdapter] Updated user', { userId: user.id });
      }
      return updated;
    },
    ...
    async deleteUser(userId: string): Promise<void> {
      ...
      if (user) {
        ...
        if (process.env.NODE_ENV === 'development') {
          console.info('[RedisAdapter] Deleted user', { userId });
        }
      }
    },
    ...
    async updateSession(
      ...
      if (process.env.NODE_ENV === 'development') {
        console.info('[RedisAdapter] Updated session', {
          sessionToken: session.sessionToken,
        });
      }
      return updated;
    ...
    async deleteSession(sessionToken: string): Promise<void> {
      ...
      await client.del(key);
      if (process.env.NODE_ENV === 'development') {
        console.info('[RedisAdapter] Deleted session', { sessionToken });
      }
    },
    ...
    async linkAccount(account: AdapterAccount): Promise<void> {
      ...
      if (process.env.NODE_ENV === 'development') {
        console.info('[RedisAdapter] Linked account', {
    ...
    async unlinkAccount({
      ...
      if (process.env.NODE_ENV === 'development') {
        console.info('[RedisAdapter] Unlinked account', { provider, providerAccountId });
      }
    },
    ...
    async createVerificationToken(
      ...
      if (process.env.NODE_ENV === 'development') {
        console.info('[RedisAdapter] Created verification token', {
    ...
    async useVerificationToken({
      ...
      if (process.env.NODE_ENV === 'development') {
        console.info('[RedisAdapter] Used verification token', { identifier });
      }
