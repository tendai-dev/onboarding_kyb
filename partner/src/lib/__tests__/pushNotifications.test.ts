...
describe('pushNotifications', () => {
  beforeEach(() => {
    ...
    (global.Notification as unknown).requestPermission = vi
      .fn()
  ...
  it('should handle permission already granted', async () => {
    (global.Notification as unknown).permission = 'granted';
    const granted = await pushNotificationService.requestPermission();
    expect(granted).toBe(true);
    expect((global.Notification as unknown).requestPermission).not.toHaveBeenCalled();
  });

  it('should handle permission denied', async () => {
    (global.Notification as unknown).permission = 'denied';
    const granted = await pushNotificationService.requestPermission();
  ...
  it('should handle browsers without notification support', async () => {
    ...
    delete (global as unknown).Notification;
    ...
    (global as unknown).Notification = originalNotification;
  });
