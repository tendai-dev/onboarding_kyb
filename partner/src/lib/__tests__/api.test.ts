...
describe('api utilities', () => {
  ...
  describe('findUserCaseByEmail', () => {
    ...
    it('should fetch user case by email', async () => {
      (global.fetch as ReturnType<typeof vi.fn>).mockResolvedValueOnce({
        ok: true,
        json: async () => ({
          items: [{ caseId: 'test-case', applicantEmail: 'test@example.com' }],
        }),
        headers: new Headers(),
      });
  ...
  describe('Messaging functions', () => {
    ...
    it('should get thread by application', async () => {
      ...
      (global.fetch as ReturnType<typeof vi.fn>).mockImplementation((__url: string) => {
        if (url.includes('/api/auth/session')) {
          return Promise.resolve({
            ok: true,
            json: async () => ({
    ...
    it('should forward message', async () => {
      ...
      await expect(
        forwardMessage('msg-1', 'thread-2', 'Forwarded')
  ...
  describe('User profile functions', () => {
    ...
    it('should update notification preferences', async () => {
      ...
      await expect(
        updateNotificationPreferences({ emailNotifications: true })
      ).resolves.toBeDefined();
    });
