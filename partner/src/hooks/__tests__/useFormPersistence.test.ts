...
describe('useFormPersistence', () => {
  ...
  describe('Field Updates', () => {
    ...
    it('should update nested field', () => {
      const { result } = renderHook(() =>
        useFormPersistence({ user: { name: '', email: '' } }, { formId: 'test-form' })
      );
  ...
  describe('Array Field Updates', () => {
    it('should add array item', () => {
      ...
      expect(
        (result.current.formData.items as Array<{ id: number; name: string }>)[0].name
      ).toBe('Item 1');
    });

    it('should remove array item', () => {
      const { result } = renderHook(() =>
        useFormPersistence({ items: [{ id: 1 }, { id: 2 }] }, { formId: 'test-form' })
      );
    ...
    it('should update array field', () => {
      const { result } = renderHook(() =>
        useFormPersistence({ items: [{ id: 1, name: 'Old' }] }, { formId: 'test-form' })
      );
  ...
  describe('Auto-save', () => {
    it('should auto-save to localStorage after delay', async () => {
      vi.useFakeTimers();
      const { result } = renderHook(() =>
        useFormPersistence({ name: '' }, { formId: 'test-form', debounceMs: 1000 })
      );
