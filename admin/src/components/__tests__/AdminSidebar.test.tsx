...
vi.mock('@/lib/mukuruImports', () => ({
  PortalNavigationSidebar: ({
    ...
    <div
      data-testid="portal-navigation-sidebar"
      onClick={() => onItemClick?.({ id: 'test' })}
      onKeyDown={(e) => {
...
describe('AdminSidebar', () => {
  ...
  it('should set active item based on work-queue pathname', () => {
    (usePathname as unknown).mockReturnValue('/work-queue');
    renderWithProviders(<AdminSidebar />);
    expect(screen.getByTestId('portal-navigation-sidebar')).toBeInTheDocument();
  });

  it('should set active item based on applications pathname', () => {
    (usePathname as unknown).mockReturnValue('/applications');
    renderWithProviders(<AdminSidebar />);
    expect(screen.getByTestId('portal-navigation-sidebar')).toBeInTheDocument();
  });
  ...
  it('should set active item for review paths', () => {
    ...
    reviewPaths.forEach((path) => {
      vi.clearAllMocks();
      (usePathname as unknown).mockReturnValue(path);
  ...
  it('should handle unknown pathname', () => {
    (usePathname as unknown).mockReturnValue('/unknown-path');
    renderWithProviders(<AdminSidebar />);
    expect(screen.getByTestId('portal-navigation-sidebar')).toBeInTheDocument();
  });

  it('should handle null pathname', () => {
    (usePathname as unknown).mockReturnValue(null);
    renderWithProviders(<AdminSidebar />);
    expect(screen.getByTestId('portal-navigation-sidebar')).toBeInTheDocument();
  });

  it('should handle empty pathname', () => {
    (usePathname as unknown).mockReturnValue('');
    renderWithProviders(<AdminSidebar />);
    expect(screen.getByTestId('portal-navigation-sidebar')).toBeInTheDocument();
  });
});
