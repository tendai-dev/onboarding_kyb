...
describe('PerformanceOptimizations Components', () => {
  ...
  describe('LazyImage', () => {
    it('should render lazy image', () => {
      renderWithProviders(<LazyImage src="/test.jpg" alt="Test image" />);
      expect(document.body).toBeInTheDocument();
    });

    it('should handle image load', async () => {
      renderWithProviders(
        <LazyImage src="/test.jpg" alt="Test image" onLoad={vi.fn()} />
      );
  ...
  describe('MemoizedTable', () => {
    ...
    it('should render memoized table', () => {
      renderWithProviders(<MemoizedTable data={mockData} columns={mockColumns} />);
      expect(document.body).toBeInTheDocument();
    });
    ...
    it('should handle sorting', () => {
      const onSort = vi.fn();
      renderWithProviders(
        <MemoizedTable data={mockData} columns={mockColumns} onSort={onSort} />
      );
      expect(document.body).toBeInTheDocument();
    });
  ...
  describe('DebouncedSearch', () => {
    it('should render debounced search', () => {
      ...
      renderWithProviders(
        <DebouncedSearch placeholder="Search..." onSearch={onSearch} />
      );
      expect(document.body).toBeInTheDocument();
    });
    ...
    it('should debounce search input', async () => {
      ...
      const { container } = renderWithProviders(
        <DebouncedSearch placeholder="Search..." onSearch={onSearch} delay={100} />
      );
  ...
  describe('useBundleOptimization', () => {
    it('should return optimization hook', () => {
      const TestComponent = () => {
        const { isOptimized } = useBundleOptimization();
        return <div>{isOptimized ? 'Optimized' : 'Not Optimized'}</div>;
      };
