...
import { FiX, FiFileText, FiEye, FiDownload, FiSave } from 'react-icons/fi';
...
export function OCRIntegration({
  ...
  entityType: _entityType = 'private_company',
  ...
  const processDocumentWithOCR = async (file: File): Promise<OCRResult> => {
    ...
    try {
      ...
      const extractedDataObj = extractedData as {
        fullText?: string;
        confidence?: number;
      ...
      const result: OCRResult = {
        ...
        fileName: file.name,
        fileType: file.type,
        extractedText: extractedDataObj.fullText || '',
        confidence: extractedDataObj.confidence || 0,
        extractedData: extractedDataObj.fields || {},
        processingTime: 3.2,
      ...
      const mappings = generateFieldMappings(result, _entityType);
      setFieldMappings(mappings);

      console.info(
        'OCR Processing Complete:',
        `Extracted data from ${file.name} with ${Math.round(result.confidence * 100)}% confidence`
      );
  ...
  const extractDataFromDocument = async (
    ...
    const mockExtractions: Record<string, unknown> = {
    ...
    const result = (mockExtractions[docType] || {
      fullText: 'Document processed successfully',
      confidence: 0.75,
      fields: {},
    }) as { fullText: string; confidence: number; fields: Record<string, unknown> };
    return result;
  };
  ...
  const handleApplyMappings = () => {
    ...
    _onDataExtracted(verifiedMappings);

    console.info(
      'Data Applied:',
      `Applied ${verifiedMappings.length} field mappings to your form`
    );
  ...
  return (
    <Box>
      {/* OCR Processing Area */}
      <Box p="6" bg={cardBg} borderRadius="lg" boxShadow="sm" mb="6">
                ...
                <VStack gap="6" align="stretch">
                  ...
                  >
                    <VStack gap="2" align="stretch">
                      <HStack justify="space-between">
                        <Text fontSize="sm" fontWeight="medium" color="mukuru.teal">
