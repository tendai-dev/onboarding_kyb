    ...
    <VStack align="stretch" gap="0">
      ...
      <Box
        ...
        dangerouslySetInnerHTML={{
          __html: value || `<div style="color: #9CA3AF;">${placeholder}</div>`,
        }}
        style={{ outline: 'none' }}
      />
    </VStack>
...
export function AdvancedDatePicker({
  ...
  const _timezones = ['UTC', 'GMT', 'EST', 'PST', 'CET', 'JST', 'AEST', 'IST'];
              ...
              <VStack align="stretch" gap="0">
                ...
                {filteredOptions.map((option) => (
                  <Box
                    key={option.value}
                    onClick={() =>
                      !option.disabled && handleSelect(option.value as string)
                    }
                    p="2"
...
export function ToastContainer({ toasts, _onClose }: ToastContainerProps) {
  return (
    <Box position="fixed" top="4" right="4" zIndex="9999" maxWidth="400px">
      <VStack align="stretch" gap="2">
        <AnimatePresence>
          {toasts.map((toast) => (
            <Toast key={toast.id} {...toast} _onClose={_onClose} />
          ))}
        </AnimatePresence>
      </VStack>
    </Box>
