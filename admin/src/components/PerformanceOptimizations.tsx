...
import { FiInfo, FiArrowUp, FiArrowDown, FiChevronDown, FiSearch } from 'react-icons/fi';
...
export function MemoizedTable<T extends { id: string }>({
  ...
  const handleSort = (key: string | keyof T) => {
    ...
    setInternalSortKey(typedKey);
    setInternalSortDirection(newDirection);
    onSort?.(typedKey, newDirection);
  };
  ...
  const renderRow = useCallback(
    (item: T, index: number) => (
      <Flex
        ...
        px="4"
        borderBottom="1px"
        borderColor={useColorModeValue('mukuru.grey.light', 'mukuru.grey.500')}
        _hover={{
          bg: useColorModeValue('mukuru.background.light', 'mukuru.background.dark'),
        }}
    ...
    return (
      <VStack gap="4" py="8">
        <Icon
          as={FiInfo}
          boxSize="8"
          color={useColorModeValue('mukuru.grey.medium', 'mukuru.grey.400')}
        />
        <Text color={useColorModeValue('mukuru.grey.mediumDark', 'mukuru.grey.300')}>
          {emptyMessage}
        </Text>
      </VStack>
    );
  ...
  return (
    ...
    >
      ...
      <Flex
        ...
        fontWeight="semibold"
        fontSize="sm"
        color={useColorModeValue('mukuru.text.primary', 'mukuru.text.inverse')}
      >
    ...
    <Box position="relative">
      ...
      {showSuggestions && suggestions.length > 0 && (
        <Box
          ...
          left="0"
          right="0"
          bg={useColorModeValue('mukuru.cards.white', 'mukuru.cards.dark')}
          border="1px"
          borderColor={useColorModeValue('mukuru.grey.light', 'mukuru.grey.500')}
          borderRadius="md"
          boxShadow="lg"
  ...
  useEffect(() => {
    ...
    const measurePerformance = () => {
      ...
      if (currentTime - lastTime >= interval) {
        ...
        const perfObj = performance as unknown as {
          memory?: { usedJSHeapSize?: number };
        };
        const memoryUsage =
          perfObj.memory && perfObj.memory.usedJSHeapSize
