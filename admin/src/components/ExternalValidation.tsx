        ...
        <VStack gap="2" align="stretch">
          <HStack gap="2">
            ...
            {result.source && (
              <Tooltip.Root>
                <Tooltip.Trigger>
                  <Icon as={FiExternalLink} boxSize="3" color="mukuru.grey.medium" />
                </Tooltip.Trigger>
                <Tooltip.Content>Validated by {result.source}</Tooltip.Content>
              </Tooltip.Root>
            )}
          </HStack>
          ...
          {result.data && typeof result.data === 'object' && result.data !== null ? (
            ...
            >
              <VStack gap="1" align="stretch">
                <Text fontSize="xs" fontWeight="medium" color="mukuru.text.primary">
                  Verified Information:
                </Text>
                {Object.entries(result.data as Record<string, unknown>).map(
                  ([key, value]) => (
                    <HStack key={key} gap="2" fontSize="xs">
                      <Text color="mukuru.grey.medium" minW="100px">
                        {key
                          .replace(/([A-Z])/g, ' $1')
          ...
          <HStack gap="2">
            <Button size="xs" variant="ghost" onClick={validateField}>
...
export function useAutoPopulation() {
  const [autoPopulatedData, setAutoPopulatedData] = useState<Record<string, unknown>>({});
