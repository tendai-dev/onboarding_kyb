...
vi.mock('@/components/Charts', () => ({
  EntityTypeChart: ({ data }: { data: unknown[] }) => (
    <div data-testid="entity-type-chart">{data.length} items</div>
  ),
  ApplicationTrendsChart: ({ data }: { data: unknown[] }) => (
    <div data-testid="trends-chart">{data.length} items</div>
  ),
  StatusPieChart: ({ data }: { data: unknown[] }) => (
    <div data-testid="status-pie-chart">{data.length} items</div>
  ),
...
describe('AdminDashboard', () => {
  ...
  beforeEach(() => {
    ...
    (
      vi.mocked(dashboardServices.fetchDashboardStats) as MockedFunction<
        typeof dashboardServices.fetchDashboardStats
      >
    ).mockResolvedValue(mockDashboardStats);
    (
      vi.mocked(dashboardServices.fetchEntityTypeDistribution) as MockedFunction<
        typeof dashboardServices.fetchEntityTypeDistribution
      >
    ).mockResolvedValue(mockEntityTypes);
    (
      vi.mocked(dashboardServices.fetchApplicationTrends) as MockedFunction<
        typeof dashboardServices.fetchApplicationTrends
      >
    ).mockResolvedValue(mockTrends);
    (
      vi.mocked(dashboardServices.fetchDashboardProjection) as MockedFunction<
        typeof dashboardServices.fetchDashboardProjection
      >
    ).mockResolvedValue({} as Partial<DashboardProjection> as DashboardProjection);
  });
  ...
  it('should handle session with email only', () => {
    (vi.mocked(useSession) as MockedFunction<typeof useSession>).mockReturnValue({
      data: { user: { email: 'user@example.com' }, expires: new Date().toISOString() },
      status: 'authenticated',
      update: vi.fn(),
    });
  ...
  it('should calculate pie chart data correctly', async () => {
    ...
    (
      vi.mocked(dashboardServices.fetchDashboardStats) as MockedFunction<
        typeof dashboardServices.fetchDashboardStats
      >
    ).mockResolvedValue(statsWithValues);
  ...
  it('should filter out zero values from pie chart', async () => {
    ...
    (
      vi.mocked(dashboardServices.fetchDashboardStats) as MockedFunction<
        typeof dashboardServices.fetchDashboardStats
      >
    ).mockResolvedValue(statsWithZeros);
  ...
  it('should extract firstName from name format', () => {
    (vi.mocked(useSession) as MockedFunction<typeof useSession>).mockReturnValue({
      data: { user: { name: 'John Doe' }, expires: new Date().toISOString() },
      status: 'authenticated',
      update: vi.fn(),
    });
