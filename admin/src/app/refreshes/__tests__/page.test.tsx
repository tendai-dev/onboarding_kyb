...
describe('RefreshesPage', () => {
  beforeEach(() => {
    ...
    (services.fetchItemsDueForRefresh as unknown).mockResolvedValue([]);
    (services.getWorkItems as unknown).mockResolvedValue({ items: [], totalCount: 0 });
  });
  ...
  it('should handle error state', async () => {
    (services.fetchItemsDueForRefresh as unknown).mockRejectedValueOnce(
      new Error('Failed')
    );
    renderWithProviders(<RefreshesPage />);
    await waitFor(() => expect(document.body).toBeInTheDocument());
