...
export default function FinalApprovalPage() {
  ...
  const loadInReviewItems = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/workqueue?status=Approved');
      if (response.ok) {
  ...
  const handleReview = (item: WorkItem) => {
    // Navigate to review page at step 4 (Final Actions)
    router.push(`/review/${item.workItemId}?step=4`);
  };
        ...
        <VStack align="stretch" gap={6}>
          ...
          <Box>
            ...
            <Text color="mukuru.text.secondary" fontSize="md">
              Applications ready for final approval or rejection
            </Text>
          </Box>
