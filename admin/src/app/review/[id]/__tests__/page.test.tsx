...
vi.mock('@/components/DocumentViewer', () => ({
  DocumentViewer: ({ isOpen, _onClose }: unknown) =>
...
describe('ReviewPage', () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    (useParams as unknown).mockReturnValue({ id: 'test-work-item-id' });
    ...
    (services.fetchMyWorkItems as unknown).mockResolvedValue({
      items: [mockWorkItem],
      totalCount: 1,
      page: 1,
      pageSize: 1000,
    });
    (services.fetchWorkItems as unknown).mockResolvedValue({
      items: [mockWorkItem],
      totalCount: 1,
      page: 1,
      pageSize: 1000,
    });
    (services.fetchWorkItemComments as unknown).mockResolvedValue([]);
    (services.fetchWorkItemHistory as unknown).mockResolvedValue([]);
  });

  it('should render loading state initially', () => {
    (services.fetchMyWorkItems as unknown).mockImplementation(
  ...
  it('should handle error state', async () => {
    ...
    (services.fetchMyWorkItems as unknown).mockRejectedValue(error);
  ...
  it('should handle approve action', async () => {
    (services.approveWorkItemUseCase as unknown).mockResolvedValue({});
  ...
  it('should handle decline action', async () => {
    (services.declineWorkItemUseCase as unknown).mockResolvedValue({});
  ...
  it('should handle add comment', async () => {
    (services.addCommentUseCase as unknown).mockResolvedValue({});
