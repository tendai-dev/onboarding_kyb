...
describe('Application Status API Route', () => {
  ...
  describe('PUT /api/applications/[id]/status', () => {
    ...
    it('should return 400 when status is missing', async () => {
      ...
      const request = createMockRequest(
        'http://localhost:3000/api/applications/test-id/status',
        {}
      );
    ...
    it('should resolve caseId to GUID before updating status', async () => {
      ...
      const request = createMockRequest(
        `http://localhost:3000/api/applications/${caseId}/status`,
        {
          status: 'IN PROGRESS',
    ...
    it('should use approve endpoint for COMPLETE status', async () => {
      ...
      const request = createMockRequest(
        `http://localhost:3000/api/applications/${guid}/status`,
        {
          status: 'COMPLETE',
          notes: 'Approved',
        }
    ...
    it('should handle GUID resolution failure gracefully', async () => {
      ...
      // Search fails
      vi.mocked(global.fetch).mockRejectedValueOnce(new Error('Search failed'));
