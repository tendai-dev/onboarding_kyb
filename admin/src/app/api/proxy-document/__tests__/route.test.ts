...
describe('Proxy Document API Route', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it('should proxy document request', async () => {
    (global.fetch as any).mockResolvedValueOnce({
      ok: true,
      blob: async () => new Blob(['test'], { type: 'application/pdf' }),
    });

    const request = createMockRequest('http://localhost:3000/api/proxy-document?documentId=doc-1');
