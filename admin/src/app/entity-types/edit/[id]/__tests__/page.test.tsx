...
describe('EditEntityTypePage', () => {
  const mockEntityType = {
    ...
    icon: 'user',
    isActive: true,
    requirements: [{ id: 'req-1', displayName: 'Requirement 1' }],
  };
  ...
  beforeEach(() => {
    ...
    (useRouter as unknown).mockReturnValue({ push: vi.fn(), back: vi.fn() });
    (entityConfigApi.entityConfigApiService.getEntityType as unknown).mockResolvedValue(
      mockEntityType
    );
    (entityConfigApi.entityConfigApiService.getRequirements as unknown).mockResolvedValue(
      [
  ...
  it('should render edit entity type page', async () => {
    renderWithProviders(
      <EditEntityTypePage params={Promise.resolve({ id: 'entity-1' })} />
    );
  ...
  it('should load entity type data', async () => {
    ...
    await waitFor(() => {
      expect(entityConfigApi.entityConfigApiService.getEntityType).toHaveBeenCalledWith(
        'entity-1',
        true
      );
  ...
  it('should handle form submission', async () => {
    (
      entityConfigApi.entityConfigApiService.updateEntityType as unknown
  ...
  it('should handle cancel', async () => {
    ...
    (useRouter as unknown).mockReturnValue(router);
  ...
  it('should handle error state', async () => {
    (
      entityConfigApi.entityConfigApiService.getEntityType as unknown
