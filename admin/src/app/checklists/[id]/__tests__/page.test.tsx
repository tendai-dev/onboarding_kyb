...
describe('ChecklistDetailPage', () => {
  ...
  beforeEach(() => {
    ...
    (checklistApi.checklistApiService.getAllChecklists as unknown).mockResolvedValue([
      mockChecklist,
    ]);
  });
  ...
  it('should render checklist detail page', async () => {
    renderWithProviders(
      <ChecklistDetailPage params={Promise.resolve({ id: 'checklist-1' })} />
    );
  ...
  it('should handle checklist not found', async () => {
    (checklistApi.checklistApiService.getAllChecklists as unknown).mockResolvedValueOnce(
      []
    );
    renderWithProviders(
      <ChecklistDetailPage params={Promise.resolve({ id: 'non-existent' })} />
    );
  ...
  it('should handle saving checklist', async () => {
    (checklistApi.checklistApiService.updateChecklist as unknown).mockResolvedValue(
      mockChecklist
    );
  ...
  it('should handle error state', async () => {
    (checklistApi.checklistApiService.getAllChecklists as unknown).mockRejectedValueOnce(
      new Error('Failed')
    );
    renderWithProviders(
      <ChecklistDetailPage params={Promise.resolve({ id: 'checklist-1' })} />
