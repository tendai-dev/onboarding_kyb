...
export async function uploadToGoogleDrive(
  ...
  // Mock successful upload - file ID and link are returned but not stored
  ...
  const mockFileId = `mock-file-${Date.now()}-${Math.random().toString(36).substring(2, 9)}`;
  const mockWebViewLink = `https://drive.google.com/file/d/${mockFileId}/view`;

  console.info(`Mock upload: ${file.name} to folder: ${folderName}`);
  console.info(`Mock file ID: ${mockFileId}`);
  console.info(`Mock web view link: ${mockWebViewLink}`);
...
export async function uploadToGoogleDrive(
  ...
  try {
    ...
    const _folderQuery = `name='${folderName}' and mimeType='application/vnd.google-apps.folder'`;
