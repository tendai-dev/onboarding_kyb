...
describe('entitySchemaRenderer', () => {
  ...
  it('should fetch entity schema', async () => {
    ...
    (entityConfigApiService.getEntityTypeByCode as unknown).mockResolvedValue(
      mockEntityType
    );
  ...
  it('should return null when entity type not found', async () => {
    (entityConfigApiService.getEntityTypeByCode as unknown).mockResolvedValue(null);
  ...
  it('should handle entity type without requirements', async () => {
    ...
    const _schema = await fetchEntitySchema('pty_ltd', {});

    expect(_schema).toBeNull();
  });
});
