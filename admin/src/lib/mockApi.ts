...
// Mock API for Mukuru KYB Application
...
// Mock data storage
const mockApplications: Application[] = generateMockApplications(50);
const mockAuditEvents: AuditEvent[] = generateMockAuditEvents(200);
...
const mockEntityTypes: EntityType[] = generateMockEntityTypes();
const mockRequirements: Requirement[] = generateMockRequirements();
...
export const mockApi = {
  ...
  getApplicationTrends: async (
    days: number = 30
  ): Promise<Array<{ date: string; applications: number; completed: number }>> => {
    await new Promise((resolve) => setTimeout(resolve, 200));
    ...
    for (let i = days - 1; i >= 0; i--) {
      ...
      const dayCompleted = mockApplications.filter(
        (app) => app.updated.startsWith(dateStr) && app.status === 'COMPLETE'
      );
  ...
  getEntityTypeDistribution: async (): Promise<
  ...
  getProcessingTimeDistribution: async (): Promise<
  ...
  // Real-time updates
  subscribeToUpdates: (callback: (data: Record<string, unknown>) => void) => {
    // Simulate real-time updates every 30 seconds
    const interval = setInterval(() => {
      // Randomly update some applications
      if (Math.random() > 0.7) {
        const randomApp =
          mockApplications[Math.floor(Math.random() * mockApplications.length)];
  ...
  createEntityType: async (
    data: Omit<EntityType, 'id' | 'createdAt' | 'updatedAt' | 'createdBy'>
  ): Promise<EntityType> => {
    await new Promise((resolve) => setTimeout(resolve, 500));
  ...
  updateEntityType: async (
    id: string,
    data: Partial<Omit<EntityType, 'id' | 'createdAt' | 'createdBy'>>
  ...
  createRequirement: async (
    data: Omit<Requirement, 'id' | 'createdAt' | 'updatedAt' | 'createdBy'>
  ): Promise<Requirement> => {
    await new Promise((resolve) => setTimeout(resolve, 500));
  ...
  updateRequirement: async (
    id: string,
    data: Partial<Omit<Requirement, 'id' | 'createdAt' | 'createdBy'>>
