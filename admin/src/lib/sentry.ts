...
export function reportError(
  ...
  if (!isSentryInitialized()) {
    ...
    if (process.env.NODE_ENV === 'development') {
      console.debug('[Sentry] Not initialized, skipping error report');
    }
    return;
  }
  ...
  try {
    ...
    // Silently fail if Sentry operations fail
    if (process.env.NODE_ENV === 'development') {
      console.debug('[Sentry] Error reporting failed:', err);
    }
  }
}
...
export function reportWarning(
  ...
  try {
    ...
    if (process.env.NODE_ENV === 'development') {
      console.debug('[Sentry] Warning reporting failed:', err);
    }
  }
}
...
export function setUserContext(user: {
  ...
  try {
    ...
    if (process.env.NODE_ENV === 'development') {
      console.debug('[Sentry] setUserContext failed:', err);
    }
  }
}
...
export function clearUserContext(): void {
  ...
  try {
    ...
    if (process.env.NODE_ENV === 'development') {
      console.debug('[Sentry] clearUserContext failed:', err);
    }
  }
}
 ...
 */
export function withErrorReportingSync<T extends (...args: unknown[]) => unknown>(
  fn: T,
  context?: {
    tags?: Record<string, string>;
    extra?: Record<string, unknown>;
