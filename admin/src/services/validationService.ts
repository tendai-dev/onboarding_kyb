  ...
  async validateCompanyRegistration(
    ...
    try {
      ...
      return {
        isValid: false,
        message:
          'Unable to verify company registration. Please check the number and try again.',
        confidence: 0,
      };
    }
  }
  ...
  async validateBatch(
    ...
    const promises = validations.map(async (validation) => {
      ...
      switch (validation.type) {
        ...
        case 'email':
          return this.validateEmail(data.email as string, validation.config);
        case 'phone':
...
export const _validationService = new ValidationService();

// Export hook for React components
export function useValidation() {
  return {
    validateCompanyRegistration:
      _validationService.validateCompanyRegistration.bind(_validationService),
    validateTaxNumber: _validationService.validateTaxNumber.bind(_validationService),
    validateBankAccount: _validationService.validateBankAccount.bind(_validationService),
    validateEmail: _validationService.validateEmail.bind(_validationService),
    validatePhoneNumber: _validationService.validatePhoneNumber.bind(_validationService),
    validateBatch: _validationService.validateBatch.bind(_validationService),
    clearCache: _validationService.clearCache.bind(_validationService),
    getCacheSize: _validationService.getCacheSize.bind(_validationService),
  };
}
