...
describe('CSV Export Utilities', () => {
  describe('escapeCsvField', () => {
    ...
    it('should escape multiple quotes', () => {
      expect(escapeCsvField('Text "with" multiple "quotes"')).toBe(
    ...
    it('should convert number to string', () => {
      expect(escapeCsvField(123 as unknown)).toBe('123');
      // Note: 0 is falsy, so it returns empty string per business logic
      expect(escapeCsvField(0 as unknown)).toBe('');
    });

    it('should handle boolean values', () => {
      expect(escapeCsvField(true as unknown)).toBe('true');
      // Note: false is falsy, so it returns empty string per business logic
      expect(escapeCsvField(false as unknown)).toBe('');
    });
  });
