...
vi.mock('../../utils/csvExport', () => ({
  ...
  createCsvBlob: (headers: string[], rows: string[][]) =>
    new Blob([headers.join(','), ...rows.map((r) => r.join(','))], { type: 'text/csv' }),
}));

// Helper function to create a complete OnboardingCaseProjection mock
function createMockProjection(
  overrides: Partial<OnboardingCaseProjection> = {}
): OnboardingCaseProjection {
  return {
    id: '1',
    caseId: '1',
...
describe('Application Use Cases', () => {
  ...
  describe('fetchApplications', () => {
    it('should fetch applications with default pagination', async () => {
      ...
      expect(applicationMapper.mapProjectionsToApplications).toHaveBeenCalledWith(
        mockProjections
      );
  ...
  describe('fetchApplicationById', () => {
    it('should fetch and map application by ID', async () => {
      ...
      vi.mocked(applicationMapper.mapProjectionToApplication).mockReturnValue(
        mockApplication
      );
      ...
      expect(applicationMapper.mapProjectionToApplication).toHaveBeenCalledWith(
        mockProjection
      );
