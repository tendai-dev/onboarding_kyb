...
export function useOfflineSubmission() {
  const [queuedSubmissions, setQueuedSubmissions] = useState<unknown[]>([]);
  ...
  const processQueuedSubmissions = useCallback(async () => {
    ...
    for (const submission of queuedSubmissions) {
      ...
      try {
        ...
        if (response.ok) {
          successfulSubmissions.push(sub.id);
          await removeQueuedSubmission(sub.id);
        }
      } catch (error) {
        console.error('Failed to process queued submission:', sub.id, error);
      }
    }
    ...
    setQueuedSubmissions((prev) =>
      prev.filter((submission) => {
        const sub = submission as { id: string };
        return !successfulSubmissions.includes(sub.id);
      })
    );
  ...
  const fetchData = useCallback(
    async (forceRefresh = false) => {
      ...
      try {
        ...
        // Cache the data
        await cacheData(key, freshData as Record<string, unknown>);
      } catch (err) {
...
// IndexedDB utilities
async function storeQueuedSubmission(submission: Record<string, unknown>): Promise<void> {
