...
public class OnboardingCase
{
    ...
    /// <summary>
    /// Transition case to Risk Review phase
    /// Called when a risk assessment is initiated for this case
    /// </summary>
    public void StartRiskReview(string initiatedBy)
    {
        // Allow transition from Submitted or UnderReview status
        if (Status != OnboardingStatus.Submitted && Status != OnboardingStatus.UnderReview)
        {
            // If already in UnderReview, allow the transition (idempotent)
            if (Status == OnboardingStatus.UnderReview)
                return;
            
            throw new InvalidOperationException($"Cannot start risk review for case in status {Status}. Case must be in Submitted or UnderReview status.");
        }
        ...
        // Store metadata to indicate risk review has started
        Metadata["risk_review_started_at"] = DateTime.UtcNow.ToString("O");
        Metadata["risk_review_initiated_by"] = initiatedBy;
