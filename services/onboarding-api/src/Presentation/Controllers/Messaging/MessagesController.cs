...
public class MessagesController : ControllerBase
{
    ...
    [ProducesResponseType(typeof(DiagnosticIdentityResponse), StatusCodes.Status200OK)]
    public IActionResult GetDiagnosticIdentity()
    {
        var userId = GetCurrentUserId();
        var userName = GetCurrentUserName();
        var userRole = GetCurrentUserRole();
        
        var emailFromHeader = HttpContext.Request.Headers.TryGetValue("X-User-Email", out var headerEmail) 
            ? headerEmail.ToString() 
            : null;
        ...
        return Ok(new DiagnosticIdentityResponse
        {
            UserId = userId,
            UserName = userName,
            UserRole = userRole.ToString(),
            EmailFromHeader = emailFromHeader,
            EmailFromClaims = emailFromClaims,
            HasJwtToken = User.Identity?.IsAuthenticated ?? false
        });
    }
...
public record DiagnosticIdentityResponse
{
