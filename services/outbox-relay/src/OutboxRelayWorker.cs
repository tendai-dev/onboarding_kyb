...
public class OutboxRelayWorker : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        var batchSize = int.Parse(_configuration["OutboxRelay:BatchSize"] ?? "100");
        var pollIntervalMs = int.Parse(_configuration["OutboxRelay:PollIntervalMs"] ?? "1000");
    ...
    private static string DetermineTopicFromEventType(string eventType)
    {
        ...
        {
            var t when t.StartsWith("Onboarding") => "onboarding.domain-events",
            var t when t.StartsWith("Document") => "documents.domain-events",
            var t when t.StartsWith("Checklist") => "checklist.domain-events",
            var t when t.StartsWith("Risk") => "risk.domain-events",
            _ => "platform.domain-events"
        };
    }
